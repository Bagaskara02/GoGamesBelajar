...
export default function MaterialModal({ isOpen, onClose, level }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-6 bg-slate-900/75 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-4xl max-h-[92vh] bg-white rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-200/80 flex flex-col overflow-hidden text-slate-800">
        
        {/* Top Header Card */}
        <div className="bg-gradient-to-r from-slate-50 via-sky-50 to-indigo-50 px-3.5 py-2.5 sm:px-6 sm:py-3.5 border-b border-slate-200/70 flex items-center justify-between gap-2">
          <div className="flex items-center space-x-2.5 sm:space-x-3.5 min-w-0 flex-1">
            <div className="w-8 h-8 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl bg-gradient-to-tr from-sky-400 to-indigo-500 shadow-md shadow-sky-500/20 flex items-center justify-center text-lg sm:text-2xl flex-shrink-0">
              🐹
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center space-x-1.5">
                <span className={`px-2 py-0.5 rounded-full text-[9px] sm:text-xs font-bold uppercase tracking-wider border ${categoryBadgeColors[level.categoryColor] || categoryBadgeColors.blue}`}>
                  Level {level.id.toString().padStart(2, '0')} • {level.category}
                </span>
                <span className="text-[10px] font-semibold text-slate-400 hidden xs:inline">FASE MATERI</span>
              </div>
              <h2 className="text-xs sm:text-lg font-black text-slate-900 tracking-tight mt-0.5 truncate">
                {level.title} <span className="text-sky-600 font-bold hidden sm:inline">— {level.subtitle}</span>
              </h2>
            </div>
          </div>
          ...
          <button
            ...
            className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-800 flex items-center justify-center transition font-bold text-xs sm:text-sm flex-shrink-0"
        ...
        {/* Tab Navigation: Segmented Control (Tidak Akan Pernah Menumpuk / Terpotong) */}
        <div className="bg-slate-100/90 p-1 sm:p-1.5 border-b border-slate-200/70 flex items-center gap-1 sm:gap-1.5">
          <button
            onClick={() => {
              soundEffects.playClick();
              setActiveTab('materi');
            }}
            className={`flex-1 py-1.5 sm:py-2 px-2 rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center space-x-1.5 transition whitespace-nowrap ${
              activeTab === 'materi'
                ? 'bg-white text-sky-600 shadow-sm border border-slate-200/80'
                : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            <BookOpen className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-sky-500 flex-shrink-0" />
            <span>Materi</span>
            <span className="hidden sm:inline">Inti</span>
          </button>
          ...
          >
            <Target className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-500 flex-shrink-0" />
            <span>Misi</span>
        ...
        <div className="flex-1 p-3.5 sm:p-6 overflow-y-auto space-y-4 sm:space-y-6 bg-slate-50/50">
          {activeTab === 'materi' && (
        ...
        <div className="bg-white px-3.5 py-2.5 sm:px-6 sm:py-3.5 border-t border-slate-200/80 flex items-center justify-between gap-3 shrink-0">
          <div className="text-[11px] sm:text-xs text-slate-500 font-medium hidden sm:block">
            Kamu selalu bisa membuka panduan ini lewat tombol <strong className="text-slate-800">&quot;Buku Materi&quot;</strong> di navbar.
          </div>

          <button
            onClick={handleStartGame}
            className="w-full sm:w-auto px-5 sm:px-7 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl bg-gradient-to-r from-sky-500 via-indigo-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white font-black text-xs sm:text-sm tracking-wide flex items-center justify-center space-x-2 shadow-lg shadow-sky-500/25 transition-all transform hover:scale-[1.02] active:scale-95"
          >
